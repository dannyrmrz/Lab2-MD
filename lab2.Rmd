...
```{r tsne}
clean_data <- na.omit(datos_pca)
tsne_model <- unique(clean_data)
tsne_thing <- Rtsne(tsne_model, dims = 2, perplexity = 30)
...
Se aplicó el algoritmo t-SNE para reducir la dimensionalidad del conjunto de datos y visualizar su estructura en dos dimensiones. Este método es útil porque permite representar datasets con muchas variables en un espacio más simple, preservando la similitud entre las observaciones.

En la visualización obtenida, cada punto representa una película del conjunto de datos. Se observan varias zonas donde los puntos se concentran, lo que sugiere la existencia de posibles agrupaciones de películas con características similares. Estas agrupaciones pueden estar relacionadas con variables como popularidad, duración, presupuesto o número de actores.


## Hallazgos y Conclusiones

El análisis realizado permitió identificar patrones claros dentro del conjunto de datos de películas. A partir del agrupamiento mediante K-means se distinguieron tres perfiles principales. El primero corresponde a producciones de alto presupuesto, elevada recaudación y gran popularidad, en su mayoría orientadas al mercado internacional y asociadas a géneros comerciales. Este grupo representa claramente el segmento de películas de alto impacto o blockbusters. El segundo perfil agrupa producciones de escala intermedia, con presupuestos y niveles de ingresos moderados, menor exposición mediática y mayor diversidad lingüística, lo cual sugiere un enfoque más regional o independiente. Finalmente, el tercer grupo está compuesto por producciones de bajo presupuesto y escasa recaudación, con niveles mínimos de popularidad, que podrían corresponder a proyectos de distribución limitada o cortometrajes.

El análisis de silueta indicó que, aunque la separación entre grupos no es perfecta, existe una estructura coherente en los datos. El método K-means presentó un desempeño ligeramente superior al clustering jerárquico, lo que justificó su selección como modelo definitivo para interpretar los segmentos identificados. En conjunto, el clustering revela que la escala financiera y el nivel de popularidad son factores determinantes en la segmentación del mercado cinematográfico.

El Análisis de Componentes Principales complementó estos hallazgos al demostrar que gran parte de la variabilidad del dataset puede explicarse mediante cuatro dimensiones principales que resumen la información contenida en doce variables originales. La primera componente está asociada al impacto comercial y a la magnitud de la producción; la segunda refleja la complejidad e internacionalización del proceso productivo; la tercera se relaciona con la estructura del elenco; y la cuarta captura elementos vinculados con la percepción pública y la popularidad. Esta reducción dimensional no solo confirma la presencia de multicolinealidad entre variables financieras y de producción, sino que también sugiere que futuros modelos predictivos podrían construirse utilizando estas componentes en lugar de todas las variables originales, reduciendo complejidad sin perder capacidad explicativa.

Las reglas de asociación reforzaron la idea de que el éxito comercial no es aleatorio. Se identificaron patrones consistentes que muestran que películas con mayor presupuesto, elevada popularidad y buena calificación promedio presentan una probabilidad significativamente más alta de generar ingresos elevados. El lift cercano a tres en varias reglas indica que estas asociaciones ocurren casi tres veces más de lo esperado bajo independencia, lo que evidencia relaciones fuertes entre inversión, percepción pública y desempeño financiero. Al eliminar variables extremadamente frecuentes como el idioma inglés o la producción en Estados Unidos, las reglas obtenidas fueron menos numerosas pero más informativas, lo que permitió identificar relaciones más relevantes desde el punto de vista analítico.

La aplicación de t-SNE permitió visualizar la estructura del dataset en dos dimensiones, mostrando concentraciones de observaciones que sugieren la existencia de agrupaciones naturales. Aunque este método no se utilizó como técnica formal de segmentación, sirvió como herramienta exploratoria que respalda visualmente los resultados obtenidos mediante clustering.

En términos estratégicos, los resultados sugieren que el desempeño financiero de una película está estrechamente vinculado a la combinación de inversión inicial, visibilidad y percepción del público. Para CineVision Studios, esto implica que una correcta asignación de presupuesto acompañada de estrategias de promoción adecuadas puede influir significativamente en el retorno económico. Asimismo, mantener un portafolio equilibrado entre producciones de alto impacto y proyectos de escala intermedia podría reducir riesgo y estabilizar ingresos.
